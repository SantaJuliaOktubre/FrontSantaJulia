...
(function init(){
  ...
  if (cart.length === 0) {
    ...
    cart.forEach(c=>{
      ...
      row.className='kv'; row.style.justifyContent='space-between'; row.style.alignItems='center'; row.style.padding='8px 0';
      row.innerHTML = `
  ...
  right.querySelector('#btn-clear').addEventListener('click', async ()=>{
    if (await confirmBox('¿Vaciar carrito?', 'Se eliminarán todos los productos')) {
      writeStore('cart', []); updateCartCount(); location.reload();
    }
  });
  ...
  function changeQty(productId, delta){
    ...
    writeStore('cart', cart); updateCartCount(); location.reload();
  }
